...
| Set<T>         | std::collections::HashSet<T>           | Unordered collection of unique elements.                             | 1                    |
| Map\<K,V>      | std::collections::HashMap\<K,V>        | Collection of key-value pairs.                                       | 1                    |
...
- **Assignments within Rules:** DDlog allows assignments using `let` or by
  ...
  `HostIP(host, addr) :- HostAddress(host, addrs), var addr = split_ip_list(addrs).flat_map(|a| Some(a))`
  3, `addr` is bound by iterating over the result of a function call.
