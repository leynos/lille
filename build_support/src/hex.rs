...
//! `digest`, and that type does not implement [`core::fmt::LowerHex`]. This
//! helper renders the raw digest bytes without pulling in a dedicated
...
/// is preallocated and each nibble maps straight into a lookup table, avoiding
/// a per-byte `format!` allocation.
///
/// # Examples
/// ```rust
/// use build_support::hex::to_lower_hex;
/// assert_eq!(to_lower_hex(&[0x00, 0x0f, 0xff]), "000fff");
/// ```
#[must_use]
...
mod tests {
    ...
    fn every_byte_renders_as_two_lowercase_round_tripping_digits() {
        ...
        // and case bugs in the low nibble.
