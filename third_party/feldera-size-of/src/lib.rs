//! `feldera-size-of`: a crate for measuring the total memory usage of an
//! object.
//!
//! This is a vendored path fork of the crates.io release `feldera-size-of`
//! 0.1.7; see `third_party/README.md` for why it exists and what was changed.
//! The upstream `#![doc = include_str!("../README.md")]` is replaced with this
//! inline module doc because the upstream `README.md` is not vendored.
//!
//! Cargo does not apply `--cap-lints allow` to path dependencies the way it
//! does for registry and git dependencies, so the upstream sources below would
