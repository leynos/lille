...
mod std {
    ...
    // The upstream `ui` trybuild test and its `src/tests/pass/*.rs` fixtures are
    // omitted from this vendored fork: the `size-of-derive` macro expands to the
    // `::size_of` crate path, which only resolves when the crate is imported
    // aliased as `size-of` (as Lille does). Built standalone the crate is
    // `feldera_size_of`, so those fixtures cannot compile here.
...
// Regression coverage for ordered-float 5.x support. `OrderedFloat<f64>` and
// `NotNan<f64>` must implement `SizeOf` so downstream `#[derive(SizeOf)]`
// records that embed them (e.g. DBSP keys) continue to compile and account for
...
mod ordered_float {
    ...
    /// Assert at compile time that a type implements [`SizeOf`].
    fn assert_size_of<T: SizeOf>() {}

    #[test]
    fn ordered_float_implements_size_of() {
        assert_size_of::<OrderedFloat<f64>>();
        assert_size_of::<OrderedFloat<f32>>();
        assert_size_of::<NotNan<f64>>();

        // An `OrderedFloat<f64>` is a transparent wrapper over an `f64`, so it
        // reports the same flat size and owns no child allocations.
        assert_eq!(OrderedFloat(0.0_f64).size_of(), TotalSize::total(size_of::<f64>()));
        assert_eq!(
            NotNan::new(1.5_f64).unwrap().size_of(),
            TotalSize::total(size_of::<f64>()),
        );
    }
}
